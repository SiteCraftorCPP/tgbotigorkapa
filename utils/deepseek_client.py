...
class DeepSeekClient:
    ...
    def _build_payload(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        system_prompt = (
            "Ты — DeepSeek: модуль СТРУКТУРНОГО АНАЛИЗА и ПОСТРОЕНИЯ РЕАЛИСТИЧЕСКОГО "
            "ТРЕЙД‑ПЛАНА поверх MEGABOT. MEGABOT уже проверил ATR, ликвидность, спреды, "
            "уровни, сигнальную свечу, тренд/структуру и RR. Ты НЕ фильтруешь рынок и НЕ "
            "дублируешь проверки MEGABOT. Твоя задача — как трейдер уровня senior "
            "оценить структуру и контекст (новости, сентимент, BTC/ETH) и построить "
            "реалистичные entry/SL/TP. Строго избегай галлюцинаций и используй только "
...
- построить реалистичные entry, SL, TP1–TP3 (TP4 строго опционален),

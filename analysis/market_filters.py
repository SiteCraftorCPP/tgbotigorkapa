...
class MarketFilters:
    ...
    async def check_all_filters(ticker: str, timeframe: str, df: pd.DataFrame, 
        ...
        # 2. Top-200 by market cap
        is_top_200 = await MarketFilters.check_top_200(ticker)
        if not is_top_200:
            result['reason'] = "Not in TOP-200 by market cap"
            return result
        
        # 3. Futures volume ≥ 3,000,000 USDT
        ...
        # 5. Liquidity ≥ 150,000 USDT within 0.3% of price
    ...
    @staticmethod
    async def check_top_200(ticker: str) -> bool:
        """
        Check if coin is in TOP-200 by market cap
        
        TODO: Integrate with CoinGecko/CoinMarketCap API
        For now simplified - consider major coins as TOP-200
        """
        # Major coins list (simplified) - TOP 200
        top_coins = [
            # Top 50
    ...
    async def check_futures_volume(ticker: str, client: XTClient) -> Optional[float]:
        """
        Check futures volume ≥ 5,000,000 USDT
        """
        try:
    ...
    async def check_liquidity(ticker: str, client: XTClient) -> Optional[float]:
        """
        Check liquidity ≥ 150,000 USDT within 0.3% of price
    ...
    def check_atr_volatility(df: pd.DataFrame) -> Dict:
        """
        ATR volatility filter (safe and simple)
        ATR % = ATR(1H) / Price × 100
        Condition: 0.5% ≤ ATR% ≤ 2.5%
